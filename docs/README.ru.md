...
Для установки навыка глобально (доступен во всех проектах):

```bash
npx skills add -g Epsilondelta-ai/flowbook
```

Устанавливает навык в глобальную директорию каждого агента (например, `~/.claude/skills/`, `~/.config/opencode/skills/` и т.д.).
