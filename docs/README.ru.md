...
flowbook skill <agent> [-g]  Установить навык AI-агента & команду /flowbook
...
Высокоуровневые навыки AI-агента

Используйте `flowbook skill` для установки навыков AI-агента и команд `/flowbook`.
...
2. Настраивает Flowbook, если он ещё не инициализирован
3. Генерирует файлы `.flow.md` с диаграммами Mermaid для каждого значимого потока
4. Проверяет сборку

### `flowbook skill`

Устанавливает навыки и команды `/flowbook` для конкретных агентов:

```bash
# Установить для конкретного агента (уровень проекта)
flowbook skill opencode
flowbook skill claude
flowbook skill cursor

# Установить для всех агентов
flowbook skill all
...
| Компонент | Описание |
|-----------|-------------|
| **Навык** (`SKILL.md`) | Автоматически активируется, когда вы упоминаете "flowbook" в промптах |
| **Команда slash** (`/flowbook`) | Явная активация — введите `/flowbook` для генерации блок-схем |

Команды slash устанавливаются для агентов, которые их поддерживают: **Claude Code**, **Cursor**, **Windsurf**, **OpenCode**.
...
> **Примечание:** `npx skills add` устанавливает только навыки (SKILL.md). Открыть команды `/flowbook` также используйте `flowbook skill`.

### Поддерживаемые агенты

| Агент | Навык | Команда Slash |
|-------|-------|---------------|
...
Если вы не использовали `flowbook skill` или `npx skills add`, скопируйте файлы вручную:
...
# Команда slash (Claude Code)
mkdir -p .claude/commands
cp node_modules/flowbook/src/commands/flowbook.md .claude/commands/
```

Замените директории на подходящие пути из таблицы выше.
...
├── node/
│   ├── cli.ts                  # Точка входа CLI (init, dev, build, skill)
│   ├── server.ts               # Программный сервер Vite и сборка
│   ├── init.ts                 # Логика инициализации проекта
│   ├── skill.ts                # Навык AI-агента & установщик команд
│   ├── discovery.ts            # Сканер файлов (fast-glob)
│   ├── parser.ts               # Объектив frontmatter + mermaid
│   └── plugin.ts               # Плагин виртуального модуля Vite
├── skills/
│   └── flowbook/
│       └── SKILL.md            # Определение навыка AI-агента
├── commands/
│   ├── flowbook.md             # Команда slash (формат frontmatter)
│   └── flowbook.plain.md       # Команда slash (чистый markdown)
└── client/
